...
$date = date('Y-m-d');
$select_win = mysqli_query($con,"SELECT * FROM user_winning_report WHERE game_name='".$game_name."' AND date='".$date."' AND (open_pana ='$open_pana' OR open_digit='$open_digit')");
...
$receiver = $username;
   ...
   $delete_win =  mysqli_query($con,"DELETE FROM user_winning_report WHERE game_name='".$game_name."' AND date='".$date."' AND (open_pana ='$open_pana' OR open_digit='$open_digit')");
}
       $delete = mysqli_query($con,"UPDATE result_chart set open_panna ='', open_digit='' WHERE id='".$id."'");
